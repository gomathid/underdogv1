              ...
              <?php
				...
				//include('jsonstring_test.php');
				  $matchArray = json_decode($json_string,true);
						$matchArray1=array();
						echo $length=count($matchArray);
						echo '<br>';
						//print_r($matchArray);
						//$current_week_arr=array('Tue Mar 13','Wed Mar 14','Fri Mar 16');
						$date = date("m/d/Y");
$ts = strtotime($date);
...
for($i = 1; $i <= 7; $i++) {
	...
	$current_week_arr[]=date("D M d", $ts);
   // print date("m/d/Y l", $ts) . "\n";
  // print date("D M d", $ts) . "\n";
   
}
echo '<pre>';
print_r($current_week_arr);
echo '</pre>';
					...
					foreach($matchArray as $mydata)
						{
						  ...
						  // $cdatetime=strtotime(date("Y-m-d H:i:s"));
										...
										if(in_array($dateLive,$current_week_arr))
										{
										$matchArray1[]=$mydata;
										
										}
										}
										echo count($matchArray1);

						
						foreach($matchArray1 as $mydata)
						{
						  ...
						  // $cdatetime=strtotime(date("Y-m-d H:i:s"));
							...
							$key = array_search($id, array_column($matchArray1, 'ID'));
							//echo $key;
							...
							$arr=json_encode($matchArray1[$key]);
