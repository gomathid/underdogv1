			...
			<div class="col-lg-16 col-md-16 col-sm-16">
                ...
                <div class="top-search-bar clearfix">
						...
						<div class="col-lg-6 text-right" >
							<button type="button" class="btn btn-success" data-toggle="modal" data-target="#create-league"><i class="fa fa-plus" aria-hidden="true"></i> Create League</button>
					...
					$sql=@mysql_query("select * from league where status=1 and ( added_user='$user_id' or id='$league_id' $leagueqry )");
							...
							while($data=@mysql_fetch_array($sql))
							{
								...
								$population=@mysql_query("select count(*) as population,leader_accept from invites where league_id='$league_id' and accepted=1 and leader_accept=1  ");
								...
								$leader_accept=$dt['leader_accept'];
								if($pcount==0)
								{
									$population=1;
								}
			...
			<div class="modal-body">
			    <form role="form" class="form-horizontal" id="create_league" method="post" action="">
					<div class="form-group">
					  <label class="col-sm-3" for="inputTo">League Name <span style="color:red">*</span></label>
					  <div class="col-sm-9"><input type="text" name="league_name" id="league_name" class="form-control validation_chk"></div>
					</div>
					<span id="league_errorMsg" class="error_msg"> </span>
					<div class="form-group">
...
<style>
 ...
 <script>
	...
	$('body').on('blur', '#league_name', function (){
	 var league_name=this.value;
		  if(league_name!='')
			{
				  $.ajax({
						type: "POST",
						url: "ajaxfunctions.php?checkLeaguename",
						...
						success: function(data) {
							...
							{
							 $('#league_name').val('');
							 $('#league_errorMsg').show();
							 $('#league_errorMsg').html('This league name was already registered.');
							 $('#league_name').focus();
							 $('#league_name').css("border-color","red");
							}
							else
							{
							$('#league_errorMsg').hide();
							$('#league_name').css("border","");	
							}
						}
				 });
	 ...
	 $('body').on('click', '.addleague', function (){	
		...
		else
		{
		 ...
		 $.ajax({
				type: "POST",
				url: "ajaxfunctions.php?create_league",
				...
				success: function(data) {
					...
					if(data=='success')
					{
						...
						$('#succ_msg_invite').html('Confirmation mail has been sent to your registered email').modal();;
					}
					else
					{
						$('.succ_msg').hide('');
						$('.alert_msg').show('').modal();
						$('#alert_msg_invite').html(data).modal();
						$("#league_name,#inputBody").val('');
					}
				}
		 });
