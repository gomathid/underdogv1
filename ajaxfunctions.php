...
if(isset($_GET['usersignup']))
{
	...
	if(($emailNum==0)&&($userNum==0))
	{
		...
		$message = "Hi&nbsp;" . $first_name . "!," . "<br/><br/>" . " Welcome to Underdog.<br><br>Now you became a member of the league   <b>".$league_name."</b><span><br/><br/>Click Below link to confirm . <br/><br/><p>".$link."</p><br/><br/>@" . date("Y") . " Underdog";
...
if(isset($_GET['create_league']))
{
	...
	if($num>0)
	{
	echo "This league name was already registered.";	
	}
	...
	else
	{
		...
		$league_id=@mysql_insert_id();
		$random_string=random_string(50);
		$link= $_SERVER['HTTP_HOST']."/projects/underdogv1/acceptleague.php?token=".$random_string."&userId=".$added_user."&league_id=".$league_id;
		
		 if($qry)
		 {
			 $update=@mysql_query("update users set login_type=1 where id='$added_user'");
			 
			 $getUser=@mysql_fetch_array(@mysql_query("select email,first_name from users  where id='$added_user'"));
	  ...
	  $subject='Welcome to Underdog';
		...
		$message = "Hi&nbsp;" . $first_name . "!," . "<br/><br/>" . " Welcome to Underdog.<br><br>You have successfully created the  league <b>".$league_name."</b> at Underdog.<span><br/><br/>Click Below link to confirm your league. <br/><br/><p>".$link."</p><br/><br/>@" . date("Y") . " Underdog";
							 ...
							 <table class="table borderless <?php  echo $class; ?>">
								...
								<tbody>
                                  <tr>
									...
									<td class="link-show"><button class="btn <?php if($pointsHome>$pointsAway){?>bet<?php } else{ echo 'unbet';} ?>" id="home" data-id="<?php echo $id; ?>"> <?php  echo $resultsLive['PointSpreadHome']; ?> (<?php  echo $resultsLive['PointSpreadHomeLine']; ?>)</button> <?php if($pointsHome>$pointsAway){?> (Underdog)<?php } ?> </td>
									
								  </tr>
                                 
								  <tr>
									<td><?php echo $mydataLive['AwayTeam'] ?></td>
									<td class="link-show"><button class="btn  <?php if($pointsAway>$pointsHome){?>bet<?php } else{ echo 'unbet';} ?>" id="away" data-id="<?php echo $id; ?>"><?php  echo $resultsLive['PointSpreadAway']; ?> (<?php  echo $resultsLive['PointSpreadAwayLine']; ?>)</button> <?php if($pointsAway>$pointsHome){ ?> (Underdog)<?php } ?> </td>
...
/*remove bet*/
if(isset($_GET['del_betting']))
{
	
	$id=$_GET['id'];
	//$delete=@mysql_query("delete from league where id='".$id."'");
	//echo "update league set status=1 where id='".$id."'";
	$delete=@mysql_query("delete from manage_league where id='".$id."'");
