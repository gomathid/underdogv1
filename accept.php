...
include('db.php');
	...
	$userCheck=@mysql_query("select * from users where id='$userId' ");
	$usernum=@mysql_num_rows($userCheck);
	if($usernum>0)
	{
		$checkDt=@mysql_fetch_array($userCheck);
	    $userstatus=$checkDt['status'];
		
		if($userstatus==0)
		{
			$inviteSql=@mysql_query("update users set status=1 , user_confirm=1 where id='$userId'");
		    $leagueSql=@mysql_query("update league  set status=1 where id='$league_id'");
		...
		{
			$msg=" <span style='color:red'>You are already a member at Underdog</span>";
		}
	}
	else
	{
