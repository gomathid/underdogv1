				...
				<div class="form-box">
                   ...
                   <!-- <h2><?php //echo $session_league_name;  ?></h2>-->
						<table class="table table-admin-style table-striped"  id="myTable">
							...
							$sql=@mysql_query("select * from league  where added_user='".$_SESSION['user_id']."'");
							while($data=@mysql_fetch_array($sql))
							{
							 $league_id=$data['id'];
							 $league_name=$data['league_name'];
							 ?>
                              <tr><td colspan="6"  style="background-color: #C4E1FF"><h2 style="color:#000;"><?php echo $data['league_name']; ?></h2></td></tr>
                             <?php
								//echo "select * from invites where league_id='$league_id' and accepted=1";
								...
								while($leagueData=@mysql_fetch_array($leagueUser)){
									...
									$userstatus=$userDt['status'];
									$user_confirm=$userDt['user_confirm'];
									$l_user_id=$userDt['id'];
									/*if($leader_accept==1)
									{
										if($userstatus==1)
							 ...
							 if($user_confirm=='1')
								{
									  if($userstatus==1)
										...
										else
										{
											...
											$action=0;
											$data_tooltip='Blocked By Admin';
										}
								}
								else
								{
									        $approveStatus='Incomplete Registration';
											$userclass='inactive-user';
											$action=0;
											$data_tooltip='Incomplete Registration';
									
								}
									}
									else
									{
										$approveStatus='Blocked for this league';
										$userclass='inactive-user';
										$action=1;
										$data_tooltip='Activate';
									}
								if($l_user_id!=$_SESSION['user_id']){
							?>
							  ...
							  <tr>
                               ...
                               <td>  <span class="<?php echo $userclass; ?>"><?php echo $approveStatus; ?></span></td>
                                
								
                                <td >
                                
                             <?php if(($leader_accept==1)&&($userstatus==1)){  ?>
                                <a href="#" data-toggle="modal"  data-target="#remove-league-user" class="edit-btn del_league_user" data-id="<?php echo $league_id; ?>" data-user="<?php echo $userDt['email']; ?>" data-tooltip="<?php echo $data_tooltip; ?>" > <span class="fa fa-ban"></span></a>
                                <?php }  else{ if($action==1){ ?>
                                
                               <a href="#" data-toggle="modal" data-target="#activate-league-user" class="edit-btn act_league_user"  data-id="<?php echo $league_id; ?>" data-user="<?php echo $userDt['email']; ?>" data-tooltip="<?php echo $data_tooltip; ?>" > <span class="fa fa-toggle-on"></span></a>
                                <?php }else{ ?> <a href="#"  class="delete-btn "  data-tooltip="<?php echo $data_tooltip; ?>" style="cursor:default" > <span class="fa fa-toggle-on"></span></a>  <?php }}  ?>   
