...
include('db.php');
	 ...
	 $check=@mysql_query("select * from league where id='$league_id' and added_user='$userId'");
	 $leaguenum=@mysql_num_rows($check);
	 $checkdt=@mysql_fetch_array($check);
	 $leagueStatus=$checkdt['status'];
	 
   if($leaguenum>0)
	{
			 if($leagueStatus==0)
			 {
				$leagueSql=@mysql_query("update league  set status=1 where id='$league_id' and added_user='$userId'");
			
				if(isset($_SESSION['user_id']))
				{
					 $msg='<span style="color:green"> Thank you for confirming your email.Click <a href="lobby.php" >here</a> to view your leagues </span>';
					$_SESSION['login_type']=1; 
				}
				else
				{
					 $msg='<span style="color:green"> Thank you for confirming your email.Click <a href="javascript:void(0);" data-toggle="modal" data-target="#myModal" id="loginlink">here</a> to login </span>';
				}
			 ...
			 {
				$msg=" <span style='color:red'>You have already confirmed this league</span>";
			 }
