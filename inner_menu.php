...
//print_r($dateRange_week);
       $weekscoreSql=@mysql_query("select * from manage_league where user_id='".$_SESSION['user_id']."' and DATE_FORMAT(added_date, '%Y-%m-%d')  in (".$datearrayStr.")");
	 ...
	 if($match_league_status!=0){ 
		 ...
		 if($count>0)
			...
			$OddType=$resultDt['odd_type'];
				...
				$resultsData=array('betTeam'=>$betTeam,"winstatus"=>$winStatus,"matchStatus"=>$matchStatus,"score"=>$score,'away_team'=>$away_team,'home_team'=>$home_team,'HomeScore'=>$HomeScore,'AwayScore'=>$AwayScore,'leaguename'=>$league_name,'added_date'=>$added_date);
...
<div class="row">
			<div class="col-lg-12">
				<ul class="nav nav-tabs">
                   ...
                   <!-- <li><a href="javascript:void(0);" data-toggle="modal" data-target="#viewScoreDetails"  class=" viewScore" >My Picks</a></li>-->
                    <li  <?php if($pagename=='leader_board.php'){echo 'class="active"'; }?>><a href="leader_board.php">League Leader Board</a></li>
